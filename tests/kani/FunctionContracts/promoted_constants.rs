...
#[cfg_attr(kani, kani::requires(foo == Foo(1)))]
...
#[cfg_attr(kani, kani::requires(foo == FOO))]
...
#[cfg_attr(kani, kani::requires(&foo == unsafe { &FOO_MUT }))]
pub fn foo_mut_static(foo: Foo) -> Foo {
    assert!(foo.0 == 1);
    foo
}

#[cfg_attr(kani, kani::proof_for_contract(foo_promoted))]
fn check_promoted() {
    foo_promoted(kani::any());
}

#[cfg_attr(kani, kani::proof_for_contract(foo_static))]
fn check_static() {
    foo_static(kani::any());
}

#[cfg_attr(kani, kani::proof_for_contract(foo_mut_static))]
